	...
	public class NewGameModel
	{
		...
		public Game ConvertToGame()
		{
			...
			var players = Players
				.Select(playerModel =>
					PlayerFactory.CreatePlayer(
						playerModel.Actions.Select(action =>
							PlayerActionFactory.CreatePlayerAction(
								action.FirstAction,
								action.SecondAction,
								action.BonusAction?.FirstAction
							)
						),
						playerModel.Index,
			...
			var bonusInternalThreats = allThreats
				.Where(threat => threat.NeedsBonusExternalThreat)
				.Select(threat => ((IThreatWithBonusThreat<ExternalThreat>)threat).BonusThreat);
			var bonusThreats = bonusExternalThreats
				.Cast<Threat>()
				.Concat(bonusInternalThreats)
				.ToList();
			var damageTokens = InitialDamageModels.ToLookup(
				model => model.ZoneLocation,
				model => model.DamageToken
			);
			return new Game(
				players,
				internalThreats,
		...
		private static IList<ExternalThreat> CreateExternalThreats(
			IEnumerable<ExternalThreatModel> threatModels,
			ZoneLocation zone
		)
		{
			return threatModels
				.Select(threatModel =>
				{
					...
					if (threat.NeedsBonusInternalThreat)
						((IThreatWithBonusThreat<InternalThreat>)threat).BonusThreat =
							InternalThreatFactory.CreateThreat<InternalThreat>(
								threatModel.BonusInternalThreat.Id
							);
					if (threat.NeedsBonusExternalThreat)
						((IThreatWithBonusThreat<ExternalThreat>)threat).BonusThreat =
							ExternalThreatFactory.CreateThreat<ExternalThreat>(
								threatModel.BonusExternalThreat.Id
							);
					return threat;
				})
				.ToList();
		...
		private static IList<InternalThreat> CreateInternalThreats(
			IEnumerable<InternalThreatModel> threatModels
		)
		{
			return threatModels
				.Select(threatModel =>
