...
namespace API.Models
{
	public class TrackSectionModel
	{
		...
		public TrackSectionModel(
			...
			IEnumerable<int> threatPositions,
			IDictionary<int, TrackBreakpointType> breakpoints
		)
		{
			TrackSpaces = Enumerable
				.Range(1 + 5 * (section.DistanceFromShip - 1), section.Length)
				.Reverse()
				.Select(space => new TrackSpaceModel
				{
					Space = space,
					HasAnyThreats = threatPositions.Contains(space),
					Breakpoint = breakpoints.ContainsKey(space)
						? breakpoints[space]
						: (TrackBreakpointType?)null
				})
				.ToList();
