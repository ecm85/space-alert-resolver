...
namespace API.Models
{
	public class GameSnapshotModel
	{
		...
		public int TotalDefeatedPoints
		{
			get { return DefeatedThreats.Sum(threat => threat.Points); }
		}
		public int TotalSurvivedPoints
		{
			get { return SurvivedThreats.Sum(threat => threat.Points); }
		}
		...
		public GameSnapshotModel(Game game, string phaseDescription)
		{
			...
			InternalThreats = internalThreatsOnTrack
				.Select(threat => new InternalThreatModel(threat))
				.ToList();
			InternalTrack = new TrackSnapshotModel(
				game.ThreatController.InternalTrack,
				threatPositions
			);
			...
			DefeatedThreats = game
				.ThreatController.DefeatedThreats.Select(threat => new ThreatModel(threat))
				.ToList();
			SurvivedThreats = game
				.ThreatController.SurvivedThreats.Select(threat => new ThreatModel(threat))
				.ToList();
			KnockedOutPlayers = game
				.Players.Where(player => player.IsKnockedOut)
				.Select(player => new PlayerModel(player))
				.ToList();
			Players = game.Players.Select(player => new PlayerModel(player)).ToList();
		}

		private static IEnumerable<InternalThreat> OrphanThreatRepresentatives(
			List<InternalThreat> internalThreats
		)
		{
