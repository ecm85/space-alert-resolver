...
namespace API.Models
{
	public abstract class StandardZoneModel
	{
		...
		protected StandardZoneModel(Game game, ZoneLocation zoneLocation)
		{
			...
			var externalThreatsInZone = game
				.ThreatController.ExternalThreatsOnTrack.Where(threat =>
					threat.CurrentZone == zoneLocation
				)
				.ToList();
			...
			ExternalThreats = externalThreatsInZone
				...
				.ToList();
			Track = new TrackSnapshotModel(
				game.ThreatController.ExternalTracks[zoneLocation],
				externalThreatPositions
			);
			Damage = zone
				.CurrentDamageTokens.Select(damage =>
					string.Format(CultureInfo.InvariantCulture, "{0}-{1}", zoneLocation, damage)
				)
				.ToList();
			UnusedDamage = zone
				.UnusedDamageTokens.Select(damage =>
					string.Format(CultureInfo.InvariantCulture, "{0}-{1}", zoneLocation, damage)
				)
				.ToList();
