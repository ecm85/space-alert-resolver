...
namespace API.Models
{
	public abstract class StandardStationModel : StationModel
	{
			...
			: base(game, station)
		{
			...
			var internalThreatInZoneModels = game
				.SittingDuck.ThreatController.InternalThreatsOnShip.Where(threat =>
					threat.DisplayOnShipStations.Contains(station)
				)
				.Select(threat => new InternalThreatInZoneModel { FileName = threat.FileName });
			var warningModels = game
				.SittingDuck.ThreatController.InternalThreatsOnShip.SelectMany(threat =>
					threat.WarningIndicatorStations
				)
				.Where(warningStation => warningStation == station)
				.Select(warningStation => new InternalThreatInZoneModel { FileName = "Warning" });
			InternalThreats = internalThreatInZoneModels.Concat(warningModels).ToList();
		}
	}
}
