...
namespace BLL.Test
{
	...
	public static class SlimeIntegrationTest
	{
		...
		*/
		...
		private static IEnumerable<PlayerActionType?> ComputerMaintenanceActions =>
		...
		public static void RedSlimeNoYCrossesNoBreakpoints_NoDamageNoDisabledBattleBots_Defeated()
		{
			var players = new List<Player>
			{
				new Player(
					...
					0,
					PlayerColor.Blue,
					PlayerSpecialization.EnergyTechnician
				),
				new Player(
					PlayerActionFactory.CreateSingleActionList(ComputerMaintenanceActions),
					1,
					PlayerColor.Green
				)
			};
		...
		{
			var redPlayerActions = PlayerActionFactory
		...
		private static void SlimeBHelper(
			bool isDefeated,
			bool isSurvived,
			TrackConfiguration internalTrack,
			int timeAppears,
			List<Player> players,
			int blueDamage,
			int redDamage,
			int whiteDamage,
			bool battleBotsDisabled
		)
		{
			var totalPoints = 0;
			if (isDefeated)
			...
			if (isSurvived)
				totalPoints = 3;
			var otherTracks = EnumFactory
				.All<TrackConfiguration>()
				.Except(new[] { internalTrack })
				.ToList();
			...
			var battleBotsAreDisabledInPlace =
				game.SittingDuck.RedZone.LowerRedStation.BattleBotsComponent.BattleBots?.IsDisabled
				?? false;
			...
			{
				var errorMessage =
					$"Survived + points didn't match. Expected: {totalPoints} points, survived: {isSurvived}. Actual: {game.TotalPoints} points, survived: {game.ThreatController.SurvivedThreats.Any()}.";
