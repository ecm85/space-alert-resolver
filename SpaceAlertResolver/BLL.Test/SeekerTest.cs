	...
	public static class SeekerTest
	{
		...
		public static void Test_MoveToMostPlayers_LowerBlueMovesUp()
		{
			Test_MoveToMostPlayers_Helper(
				new Dictionary<StationLocation, int>
				{
					{ StationLocation.LowerWhite, 2 },
					{ StationLocation.UpperBlue, 3 }
				},
		...
		public static void Test_MoveToMostPlayers_LowerBlueMovesRed()
		{
			Test_MoveToMostPlayers_Helper(
				new Dictionary<StationLocation, int>
				{
					{ StationLocation.LowerWhite, 3 },
					{ StationLocation.UpperBlue, 2 }
				},
				StationLocation.LowerBlue,
		...
		public static void Test_MoveToMostPlayers_LowerWhiteMovesUp()
		{
			Test_MoveToMostPlayers_Helper(
				new Dictionary<StationLocation, int>
				{
					{ StationLocation.LowerBlue, 2 },
					{ StationLocation.UpperWhite, 3 },
					{ StationLocation.LowerRed, 2 }
				},
				StationLocation.LowerWhite,
		...
		public static void Test_MoveToMostPlayers_LowerWhiteMovesBlue()
		{
			Test_MoveToMostPlayers_Helper(
				new Dictionary<StationLocation, int>
				{
					{ StationLocation.LowerBlue, 5 },
					{ StationLocation.UpperWhite, 4 },
					{ StationLocation.LowerRed, 4 }
				},
				StationLocation.LowerWhite,
				StationLocation.LowerBlue
		...
		private static void Test_MoveToMostPlayers_Helper(
			Dictionary<StationLocation, int> countsByLocation,
			StationLocation currentStation,
			StationLocation expectedNewStation
		)
		{
			var seeker = new Seeker { CurrentStation = currentStation };
			seeker.SetInitialPlacement(4);
			var threatController = new ThreatController(
				null,
				null,
				new List<ExternalThreat>(),
				new List<InternalThreat>(),
				null
			);
			var mockSittingDuck = new Mock<SittingDuck>(
				MockBehavior.Strict,
				threatController,
				null,
				null
			...
			foreach (var countByLocation in countsByLocation)
			{
				...
				mockSittingDuck.Setup(f => f.GetPlayerCount(location)).Returns(count).Verifiable();
				mockSittingDuck.Setup(f => f.RedDoorIsSealed).Returns(false);
				mockSittingDuck.Setup(f => f.BlueDoorIsSealed).Returns(false);
			}
