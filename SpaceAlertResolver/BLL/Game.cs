	...
	public class Game
	{
		...
		public void PerformTurn()
		{
			try
			{
				...
				if (
					SittingDuck.WhiteZone.UpperWhiteStation.ComputerComponent.ShouldCheckComputer(
						CurrentTurn
					)
				)
					CheckForComputer();
		...
		private void PerformEndOfGame()
		{
			...
			var rocketFiredLastTurn = SittingDuck
				.BlueZone
				.LowerBlueStation
				.RocketsComponent
				.RocketFiredLastTurn;
			if (rocketFiredLastTurn != null)
				ResolveDamage(
					new[] { rocketFiredLastTurn.PerformAttack(null) },
					new List<PlayerInterceptorDamage>()
				);
			...
			var playersInFarInterceptors = SittingDuck
				.InterceptorStations.Where(station =>
					station.StationLocation.DistanceFromShip() > 1
				)
		...
		{
			TotalPoints += SittingDuck
				.WhiteZone
				.LowerWhiteStation
				.VisualConfirmationComponent
				.TotalVisualConfirmationPoints;
		...
		private void CheckForComputer()
		{
			PhaseStarting(
				this,
				new PhaseEventArgs { PhaseHeader = ResolutionPhase.ComputerCheck.GetDescription() }
			);
			SittingDuck.WhiteZone.UpperWhiteStation.ComputerComponent.PerformComputerCheck(
				Players,
				CurrentTurn
			);
		}
		...
		{
			var interceptorDamages = SittingDuck
				.InterceptorStations.Select(station => station.PlayerInterceptorDamage)
		...
		{
			var damages = SittingDuck
				.StandardStationsByLocation.Values.Select(station => station.CurrentPlayerDamage())
		...
		private bool TargetingAssistanceProvided
		{
			get
			{
				var anyPlayersInInterceptors = SittingDuck
					.InterceptorStations.SelectMany(station => station.Players)
					.Any();
				var visualConfirmationPerformed =
					SittingDuck
						.WhiteZone
						.LowerWhiteStation
						.VisualConfirmationComponent
						.NumberOfConfirmationsThisTurn > 0;
				var targetingAssistanceProvided =
					anyPlayersInInterceptors || visualConfirmationPerformed;
		...
		)
		{
			PhaseStarting(
				this,
				new PhaseEventArgs { PhaseHeader = ResolutionPhase.ResolveDamage.GetDescription() }
			);
			...
			{
				var priorityThreatsInRange = ThreatController
					.DamageableExternalThreats.Where(threat =>
						threat.IsPriorityTargetFor(damage) && threat.CanBeTargetedBy(damage)
					)
					.ToList();
				var threatsInRange = ThreatController
					.DamageableExternalThreats.Where(threat => threat.CanBeTargetedBy(damage))
					.ToList();
		...
		{
			var threatsHitByInterceptors = ThreatController
				.DamageableExternalThreats.Where(threat =>
					threat.CanBeTargetedBy(interceptorDamages.MultipleDamage)
				)
				.ToList();
			if (threatsHitByInterceptors.Any())
			{
				var damageType =
					threatsHitByInterceptors.Count == 1
