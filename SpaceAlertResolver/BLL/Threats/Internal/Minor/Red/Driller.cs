	...
	public class Driller : MinorRedInternalThreat
	{
		...
		internal void MoveTowardsMostDamagedZone()
		{
			...
			var zones = new[]
			{
				...
				new
				{
					NewZone = currentZone.BluewardZoneLocation(),
					MoveCommand = new Action(MoveBlue)
				},
				new
				{
					NewZone = currentZone.BluewardZoneLocation().BluewardZoneLocation(),
					MoveCommand = new Action(MoveBlue)
				},
				new
				{
					NewZone = currentZone.RedwardZoneLocation(),
					MoveCommand = new Action(MoveRed)
				},
				new
				{
					NewZone = currentZone.RedwardZoneLocation().RedwardZoneLocation(),
					MoveCommand = new Action(MoveRed)
				}
			}
			...
			var mostDamagedZoneGroup = zones
				...
				{
					Zone = zone,
					DamageTaken = SittingDuck.GetDamageToZone(zone.NewZone.Value)
				})
