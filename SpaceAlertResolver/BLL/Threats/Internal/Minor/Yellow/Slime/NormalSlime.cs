...
namespace BLL.Threats.Internal.Minor.Yellow.Slime
{
	...
	{
		protected NormalSlime(StationLocation stationLocation)
			: base(2, stationLocation)
		{
		...
		public override IList<StationLocation> DisplayOnTrackStations
		{
			...
			{
				return base
					.DisplayOnTrackStations.Concat(
						Progeny
							.Where(progeny => progeny.IsOnShip)
							.SelectMany(progeny => progeny.CurrentStations)
					)
					.ToList();
			}
		}

		protected override bool IsDefeatedWhenHealthReachesZero
		{
			get { return Progeny.All(progeny => !progeny.IsOnShip); }
		}
		...
		public void SpreadFrom(StationLocation spreadFromStation, int position)
		{
			...
			if (
				spreadToStation == null
				|| ThreatController.DamageableInternalThreats.Any(threat =>
					threat is BaseSlime && threat.CurrentStation == spreadToStation
				)
			)
				return;
