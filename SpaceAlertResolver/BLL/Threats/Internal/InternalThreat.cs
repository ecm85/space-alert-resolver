...
namespace BLL.Threats.Internal
{
	public abstract class InternalThreat : Threat
	{
		...
		public virtual IList<StationLocation> DisplayOnTrackStations =>
			CurrentStations.Concat(WarningIndicatorStations).ToList();
		public virtual IList<StationLocation> DisplayOnShipStations => CurrentStations;
		public IList<StationLocation> WarningIndicatorStations { get; } =
		...
		public override ThreatDamageType StandardDamageType { get; } =
			ThreatDamageType.IgnoresShields;
		...
		protected InternalThreat(
			...
			ThreatDifficulty difficulty,
			int health,
			int speed,
			IList<StationLocation> currentStations,
		...
		protected InternalThreat(
			...
			: this(
				...
				health,
				speed,
				new List<StationLocation> { currentStation },
				actionType
			) { }
		...
		)
		{
			...
			if (RemainingInaccessibility.HasValue)
			{
				...
				RemainingInaccessibility =
					RemainingInaccessibility < 0 ? 0 : RemainingInaccessibility;
		...
		private void MoveToNewStation(StationLocation? newStation)
		{
			if (CurrentStations.Count != 1)
				throw new InvalidOperationException(
					"Cannot move a threat that exists in more than 1 zone."
				);
		...
		public virtual bool CanBeTargetedBy(
			StationLocation stationLocation,
		...
		)
		{
			return IsDamageable
				...
				&& CurrentStations.Contains(stationLocation);
		}
