...
namespace BLL.Threats.Internal.Serious.Yellow
{
	public class Seeker : SeriousYellowInternalThreat
	{
		...
		{
			var adjacentStations = new[]
			{
				new
				{
					NewStation = CurrentStation.RedwardStationLocation(),
					MoveCommand = new Action(MoveRed)
				},
				new
				{
					NewStation = CurrentStation.BluewardStationLocation(),
					MoveCommand = new Action(MoveBlue)
				},
				new
				{
					NewStation = CurrentStation.OppositeStationLocation(),
					MoveCommand = new Action(ChangeDecks)
				}
			...
			var adjacentStationGroupWithMostPlayers = adjacentStations
				...
				{
					Station = station,
					PlayerCount = SittingDuck.GetPlayerCount(station.NewStation.Value)
				})
