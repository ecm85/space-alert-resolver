...
namespace BLL.Threats
{
	public abstract class Threat
	{
		...
		public event EventHandler<ThreatDamageEventArgs> AttackedSittingDuck = (
			sender,
			args
		) => { };
		...
		public virtual int PointsForDefeating =>
			ThreatPoints.GetPointsForDefeating(ThreatType, Difficulty);

		protected virtual int PointsForSurviving =>
			ThreatPoints.GetPointsForSurviving(ThreatType, Difficulty);
		...
		protected int AttackSpecificZone(
			int amount,
			ZoneLocation zone,
			ThreatDamageType? threatDamageType = null
		)
		{
		...
		protected void AttackOtherTwoZones(int amount, ThreatDamageType? threatDamageType = null)
		{
			AttackSpecificZones(
				amount,
				EnumFactory.All<ZoneLocation>().Except(new[] { CurrentZone }).ToList(),
				threatDamageType
			);
		}
		...
		)
		{
			...
			foreach (var zoneLocation in zones)
			{
				...
				var damage = new ThreatDamage(
					amount + bonusAttacks,
					threatDamageType ?? StandardDamageType,
					zoneLocation,
					DamageDistanceToSource
				);
