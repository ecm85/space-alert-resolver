...
namespace BLL.Players
{
	public class Player
	{
		...
		public void ShiftFromPlayerActions(int turn)
		{
			...
			var firstActionWasStarted =
				currentAction.FirstActionSegment.SegmentStatus != PlayerActionStatus.NotPerformed;
			var secondActionWasStarted =
				currentAction.SecondActionSegment.SegmentType == null
				|| currentAction.SecondActionSegment.SegmentStatus
					!= PlayerActionStatus.NotPerformed;
			...
			{
				var actionToShift = new PlayerAction(
					currentAction.SecondActionSegment.SegmentType,
					null,
					null
				);
				currentAction.SecondActionSegment.SegmentType = null;
		...
		private void Shift(int turn, PlayerAction actionToInsert)
		{
			...
			var endTurn = turn;
			while (
				endTurn < ActionsList.Count
				&& GetActionForTurn(endTurn).FirstActionSegment.SegmentType.HasValue
			)
				endTurn++;
			ActionsList.Insert(
				turn - 1,
				new PlayerAction(
					actionToInsert?.FirstActionSegment.SegmentType,
					actionToInsert?.SecondActionSegment.SegmentType,
					actionToInsert?.BonusActionSegment.SegmentType
				)
			);
		...
		{
			return Specialization == PlayerSpecialization.Medic
				&& IsPerformingAdvancedSpecialization(currentTurn);
		}

		private bool IsPerformingAdvancedSpecialization(int currentTurn)
		{
			return GetActionForTurn(currentTurn).BonusActionSegment.SegmentType
					== PlayerActionType.AdvancedSpecialization
				|| GetActionForTurn(currentTurn).FirstActionSegment.SegmentType
					== PlayerActionType.AdvancedSpecialization;
		}
		...
		{
			return Specialization == PlayerSpecialization.Medic
				&& IsPerformingBasicSpecialization(currentTurn);
		}

		public bool IsPerformingAdvancedSpecialOps(int currentTurn)
		{
			return Specialization == PlayerSpecialization.SpecialOps
				&& IsPerformingAdvancedSpecialization(currentTurn);
		}

		public void PadPlayerActions(int numberOfTurns)
		{
			var extraNullActions = Enumerable.Repeat(
				PlayerActionFactory.CreateEmptyAction(),
				numberOfTurns - ActionsList.Count
			);
