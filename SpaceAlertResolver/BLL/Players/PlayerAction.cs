...
namespace BLL.Players
{
	public class PlayerAction
	{
		...
		private IEnumerable<PlayerActionSegment> AllSegments =>
			new[] { FirstActionSegment, SecondActionSegment, BonusActionSegment };

		private PlayerActionSegment NextUnperformedSegment =>
			AllSegments.First(segment => segment.SegmentStatus != PlayerActionStatus.Performed);
		...
		internal PlayerAction(
			PlayerActionType? firstActionType,
			PlayerActionType? secondActionType,
			PlayerActionType? bonusActionType
		)
		{
			FirstActionSegment = new PlayerActionSegment
			{
				SegmentType = firstActionType,
				SegmentStatus = PlayerActionStatus.NotPerformed
			};
			SecondActionSegment = new PlayerActionSegment
			{
				SegmentType = secondActionType,
				SegmentStatus = PlayerActionStatus.NotPerformed
			};
			BonusActionSegment = new PlayerActionSegment
			{
				SegmentType = bonusActionType,
				SegmentStatus = PlayerActionStatus.NotPerformed
			};
		}
		...
		{
			return AllSegments.All(segment =>
				segment.SegmentStatus == PlayerActionStatus.Performed
			);
		}
	}
