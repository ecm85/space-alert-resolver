...
namespace BLL
{
	public static class GameParser
	{
		public static Game ParseArgsIntoGame(IList<string> args)
		{
			...
			if (
				chunks.Count != 5
				|| chunkIdentifiers.Except(chunks.Select(chunk => chunk[0])).Any()
			)
				...
				switch (chunkType)
				{
					...
					case "-external-threats":
						...
						bonusThreats = bonusThreats
							.Concat(externalThreatResult.BonusThreats)
							.ToList();
						break;
					case "-internal-threats":
						...
						bonusThreats = bonusThreats
							.Concat(internalThreatResult.BonusThreats)
							.ToList();
						break;
				}
			}
			...
			if (internalThreats == null)
				throw new InvalidOperationException(
					"Could not find internal threats in game string."
				);
			if (externalThreats == null)
				throw new InvalidOperationException(
					"Could not find external threats in game string."
				);
			if (externalTracksByZone == null)
				throw new InvalidOperationException(
					"Could not find external tracks in game string."
				);
			if (players == null)
		...
		private static IList<Player> ParsePlayers(IList<string> chunk)
		{
			...
			while (playerTokens.Any())
			{
				...
				var player = new Player(
					PlayerActionFactory.CreateSingleActionList(actions),
					index.Value,
					color.Value
				);
				players.Add(player);
		...
		private static readonly IDictionary<char, PlayerActionType?> actionTypesByCode =
			new Dictionary<char, PlayerActionType?>
			{
		...
		private static ParseThreatsResult<ExternalThreat> ParseExternalThreats(
		...
		)
		{
			...
			while (threatTokens.Any())
			{
				...
				if (nextToken == null)
					throw new InvalidOperationException(
						"Error on external threat #" + (externalThreats.Count + 1)
					);
				switch (nextToken.Item1)
				{
					case "id":
						...
						{
							Threat = ExternalThreatFactory.CreateThreat<ExternalThreat>(
								nextToken.Item2
							)
						};
					...
					case "extra-internal-threat-id":
						bonusThreats.Add(
							SetExtraInternalThreat(
								nextToken.Item2,
								nextThreatInfo,
								externalThreats.Count + 1
							)
						);
						break;
					case "extra-external-threat-id":
						bonusThreats.Add(
							SetExtraExternalThreat(
								nextToken.Item2,
								nextThreatInfo,
								externalThreats.Count + 1
							)
		...
		private static ParseThreatsResult<InternalThreat> ParseInternalThreats(
			IEnumerable<string> chunk
		)
		{
			...
			while (threatTokens.Any())
			{
				...
				if (nextToken == null)
					throw new InvalidOperationException(
						"Error on internal threat #" + (internalThreats.Count + 1)
					);
				switch (nextToken.Item1)
				{
					case "id":
						...
						{
							Threat = InternalThreatFactory.CreateThreat<InternalThreat>(
								nextToken.Item2
							)
						};
						if (nextThreatInfo.Threat == null)
			...
			if (nextThreatInfo != null)
				throw new InvalidOperationException(
					"Error on external threat #" + (internalThreats.Count + 1)
				);
		...
		private static ExternalThreat SetExtraExternalThreat<T>(
			...
			int currentThreatIndex
		)
			where T : Threat
		{
			var bonusExternalThreat = ExternalThreatFactory.CreateThreat<ExternalThreat>(
				extraExternalThreatId
			);
			if (nextThreatInfo == null || bonusExternalThreat == null)
				throw new InvalidOperationException(
					"Error on external threat #" + currentThreatIndex
				);
		...
		private static InternalThreat SetExtraInternalThreat<T>(
			...
			where T : Threat
		{
			var bonusInternalThreat = InternalThreatFactory.CreateThreat<InternalThreat>(
				extraInternalThreatId
			);
		...
		private static void SetLocation(
			ExternalThreatInfo nextThreatInfo,
			string location,
			int currentThreatIndex
		)
		{
		...
		private static void SetTimeAppears<T>(
			StandardThreatInfo<T> nextThreatInfo,
			string timeAppears,
			int currentThreatIndex
		)
		...
		private static IDictionary<ZoneLocation, TrackConfiguration> ParseExternalTracks(
			IList<string> chunk
		)
		{
		...
		private static void InitializeBonusThreats<T>(ThreatInfo<T> threatInfo)
			where T : Threat
		{
			if (threatInfo == null)
			...
			if (threatInfo.Threat.NeedsBonusInternalThreat)
			{
				...
				((IThreatWithBonusThreat<InternalThreat>)threatInfo.Threat).BonusThreat = threatInfo
					.BonusInternalThreatInfo
					.Threat;
			}
			if (threatInfo.Threat.NeedsBonusExternalThreat)
			{
				InitializeBonusThreats(threatInfo.BonusExternalThreatInfo);
				((IThreatWithBonusThreat<ExternalThreat>)threatInfo.Threat).BonusThreat = threatInfo
					.BonusExternalThreatInfo
					.Threat;
			}
		}

		private static void AddBonusThreatInfo<T>(
			ThreatInfo<T> threatInfo,
			ExternalThreat newThreat
		)
			where T : Threat
		{
		...
		private static IEnumerable<IList<string>> ChunkArguments(
			IList<string> arguments,
			params string[] splitters
		)
		{
		...
		private static Tuple<
			ZoneLocation,
			TrackConfiguration
		> ParseZoneLocationWithTrackConfiguration(string token)
		{
		...
		private class StandardThreatInfo<T> : ThreatInfo<T>
			where T : Threat
		{
		...
		private class ParseThreatsResult<T>
			where T : Threat
		{
