...
namespace BLL
{
	public class ThreatController
	{
		...
		private readonly IList<ThreatStatus> singleTurnExternalThreatStatusEffects =
			new List<ThreatStatus>();
		...
		public IEnumerable<Threat> DefeatedThreats
		{
			get
			{
				return new List<Threat>()
					...
					.Where(threat => threat.IsDefeated);
			}
		}
		...
		public IEnumerable<Threat> SurvivedThreats
		{
			get
			{
				return new List<Threat>()
					.Concat(ExternalThreats)
					.Concat(InternalThreats)
					.Where(threat => threat.IsSurvived);
			}
		}
		...
		public int TotalThreatPoints
		{
			get
			{
				return new List<Threat>()
					.Concat(ExternalThreats)
					.Concat(InternalThreats)
					.Sum(threat => threat.Points);
			}
		}

		public IEnumerable<InternalThreat> InternalThreatsOnShip
		{
			get { return InternalThreats.Where(threat => threat.IsOnShip); }
		}
		...
		public void AddNewThreatsToTracks(int currentTurn)
		{
			PhaseStarting(
				this,
				new PhaseEventArgs { PhaseHeader = ResolutionPhase.AddNewThreats.GetDescription() }
			);

			var newExternalThreats = ExternalThreats
				.Where(threat => threat.TimeAppears == currentTurn)
				.ToList();
			foreach (var newThreat in newExternalThreats)
			{
				newThreat.AttackedSittingDuck += (sender, args) =>
				{
					ThreatAttackedShip(sender, args);
				};
				newThreat.PlaceOnTrack(ExternalTracks[newThreat.CurrentZone]);
				EventMaster.LogEvent(
					string.Format(CultureInfo.CurrentCulture, "{0} Appeared", newThreat.DisplayName)
				);
			}

			var newInternalThreats = InternalThreats
				.Where(threat => threat.TimeAppears == currentTurn)
				.ToList();
		...
		public void MoveThreats(int currentTurn)
		{
			...
			PhaseStarting(
				this,
				new PhaseEventArgs { PhaseHeader = ResolutionPhase.MoveThreats.GetDescription() }
			);
		...
		{
			foreach (
				var externalThreat in MoveableExternalThreats
					.Except(new[] { source })
					.OrderBy(threat => threat.TimeAppears)
			)
		...
		{
			foreach (
				var internalThreat in MoveableInternalThreats.OrderBy(threat => threat.TimeAppears)
			)
				internalThreat.Move(currentTurn, amount);
		}

		public void MoveExternalThreatsInZone(
			int currentTurn,
			int amount,
		...
		public void PerformEndOfTurn()
		{
			...
			foreach (
				var singleTurnExternalThreatStatusEffect in singleTurnExternalThreatStatusEffects
			)
		...
		)
		{
			...
			foreach (
				var threat in externalThreatStatusEffects.Concat(
					singleTurnExternalThreatStatusEffects
				)
			)
