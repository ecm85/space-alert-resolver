...
namespace BLL
{
	public interface ISittingDuck
	{
		...
		void RemoveZoneDebuffForSource(
			IEnumerable<ZoneLocation> zoneLocations,
			InternalThreat source
		);
		...
		void ShiftPlayersAfterPlayerActions(
			IEnumerable<ZoneLocation> zoneLocations,
			int turnToShift
		);
		void ShiftPlayersAfterPlayerActions(
		...
		void ShiftPlayersAndRepeatPreviousAction(
			IEnumerable<StationLocation> stationLocations,
			int turnToShift
		);
		...
		void UnsubscribeFromMovingOut(
			IEnumerable<StationLocation> stationLocations,
			EventHandler<PlayerMoveEventArgs> handler
		);

		void AddIrreparableMalfunctionToStations(
			IEnumerable<StationLocation> stationLocations,
