...
namespace BLL
{
	public static class MovementController
	{
		...
		)
		{
			...
			var newStation = standardStationsByLocation[
				player.CurrentStation.StationLocation.OppositeStationLocation().GetValueOrDefault()
			];
			if (newStation != null)
			{
				var movedOut = oldStation.PerformMoveOutTowardsOppositeDeck(
					player,
					currentTurn,
					false
				);
		...
		)
		{
			...
			var oldStation = standardStationsByLocation[player.CurrentStation.StationLocation];
			var newStation = standardStationsByLocation[
				player.CurrentStation.StationLocation.BluewardStationLocation().GetValueOrDefault()
			];
			if (newStation != null)
			{
		...
		)
		{
			...
			var oldStation = standardStationsByLocation[player.CurrentStation.StationLocation];
			var newStation = standardStationsByLocation[
				player.CurrentStation.StationLocation.RedwardStationLocation().GetValueOrDefault()
			];
			if (newStation != null)
			{
		...
		)
		{
			...
			var movedOut = MoveOut(
				performingPlayer,
				currentTurn,
				currentStation,
				firstDestination.Station.StationLocation
			);
			if (!movedOut)
				return;
			var finalDestination = path.First(pathLocation =>
				pathLocation.Index >= path.Count - 1
				|| !CanMoveOut(
					pathLocation.Station,
					path[pathLocation.Index + 1].Station.StationLocation
				)
			);
		...
		)
		{
			if (toStationLocation == fromStation.StationLocation.OppositeStationLocation())
				return fromStation.PerformMoveOutTowardsOppositeDeck(
					performingPlayer,
					currentTurn,
					true
				);
			...
			throw new InvalidOperationException(
				"Could not find a path to heroically move out of current station"
			);
		}
		...
		)
		{
			...
			throw new InvalidOperationException(
				"Could not determine if player could heroically move out of current station"
			);
		}

		internal static IEnumerable<StationLocation> Path(
			StationLocation fromLocation,
			StationLocation toLocation
		)
		{
		...
		private static IList<StationLocation> BluewardPath(
			StationLocation fromLocation,
			StationLocation toLocation
		)
		{
			...
			if (oppositeDeckPath != null)
				...
				{
					bluewardLocation.Value,
					oppositeDeckPath.Single()
				};
			var bluewardPath = BluewardPath(bluewardLocation.Value, toLocation);
			return bluewardPath == null
				? null
				: new[] { bluewardLocation.Value }
					.Concat(BluewardPath(bluewardLocation.Value, toLocation))
					.ToList();
		}

		private static IList<StationLocation> RedwardPath(
			StationLocation fromLocation,
			StationLocation toLocation
		)
		{
			...
			if (oppositeDeckPath != null)
				return new List<StationLocation>
				{
					redwardLocation.Value,
					oppositeDeckPath.Single()
				};
			var redwardPath = RedwardPath(redwardLocation.Value, toLocation);
			return redwardPath == null
				? null
				: new[] { redwardLocation.Value }
					.Concat(RedwardPath(redwardLocation.Value, toLocation))
					.ToList();
		}

		private static IEnumerable<StationLocation> OppositeDeckPath(
			StationLocation fromLocation,
			StationLocation toLocation
		)
		{
