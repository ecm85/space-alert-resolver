...
namespace BLL.ShipComponents
{
	public class InterceptorsInSpaceComponent : ICharlieComponent
	{
		...
		private Station SpacewardStation
		{
			get
			{
				return sittingDuck.StationsByLocation[
					stationLocation.SpacewardLocation().GetValueOrDefault()
				];
			}
		}
		...
		private Station ShipwardLocation
		{
			...
			{
				return sittingDuck.StationsByLocation[
					stationLocation.ShipwardLocation().GetValueOrDefault()
				];
			}
		}
		...
		public void PerformCAction(Player performingPlayer, int currentTurn, bool isAdvancedUsage)
		{
			...
			var currentDistanceFromShip =
				performingPlayer.CurrentStation.StationLocation.DistanceFromShip();
