...
namespace BLL.ShipComponents
{
	public abstract class StandardStation : Station
	{
		...
		{
			return IrreparableMalfunctions.Any(malfunction =>
				malfunction.ActionType == playerActionType
			);
		}
		...
		private bool CanFireCannon(Player performingPlayer)
		{
			...
			return firstAThreat == null
				&& !HasIrreparableMalfunctionOfType(PlayerActionType.Alpha)
				&& AlphaComponent.CanFire();
		}
		...
		private bool CanUseCharlieComponent(Player performingPlayer)
		{
			...
			return firstCThreat == null
				&& !HasIrreparableMalfunctionOfType(PlayerActionType.Charlie)
				&& CharlieComponent.CanPerformCAction(performingPlayer);
		}
		...
		public bool PerformMoveOutTowardsOppositeDeck(
		...
		private void UseBattleBots(Player performingPlayer, bool isHeroic)
		{
			if (performingPlayer.BattleBots == null || performingPlayer.BattleBots.IsDisabled)
				return;
			var firstBattleBotThreat = GetFirstThreatOfType(
		...
		public override void PerformNextPlayerAction(Player performingPlayer, int currentTurn)
		{
			...
			switch (actionType)
			{
				...
				case PlayerActionType.MoveBlue:
					MovementController.MoveBlue(
						SittingDuck.StandardStationsByLocation,
						performingPlayer,
						currentTurn
					);
					break;
				case PlayerActionType.MoveRed:
					MovementController.MoveRed(
						SittingDuck.StandardStationsByLocation,
						performingPlayer,
						currentTurn
					);
				...
				case PlayerActionType.TeleportBlueLower:
					MovementController.MoveHeroically(
						SittingDuck.StandardStationsByLocation,
						performingPlayer,
		...
		private void PerformBasicSpecialization(Player performingPlayer, int currentTurn)
		{
			switch (performingPlayer.Specialization)
			{
				case PlayerSpecialization.DataAnalyst:
					SittingDuck
						.StandardStationsByLocation[StationLocation.UpperWhite]
						.PerformCAction(performingPlayer, currentTurn, true);
					performingPlayer.BonusPoints++;
					break;
				case PlayerSpecialization.EnergyTechnician:
					SittingDuck
						.StandardStationsByLocation[StationLocation.LowerWhite]
						.PerformBAction(performingPlayer, false, true);
					break;
				...
				case PlayerSpecialization.Rocketeer:
					SittingDuck
						.StandardStationsByLocation[StationLocation.LowerBlue]
						.PerformCAction(performingPlayer, currentTurn, true);
					break;
				case PlayerSpecialization.SpecialOps:
					var actionToMakeHeroic = performingPlayer
						.Actions.Skip(currentTurn)
						.FirstOrDefault(action => action.CanBeMadeHeroic());
				...
				case PlayerSpecialization.Teleporter:
					var playerToTeleport = SittingDuck.PlayersOnShip.SingleOrDefault(playerOnShip =>
						playerOnShip.PlayerToTeleport
					);
					var teleportDestination = SittingDuck.PlayersOnShip.SingleOrDefault(
						playerOnShip => playerOnShip.TeleportDestination
					);
				...
				default:
					throw new InvalidOperationException(
						"Missing specialization when attempting basic specialization."
					);
			}
		}
		...
		private void BasicHypernavigator()
		{
			if (StationLocation.IsLowerDeck())
				SittingDuck.ThreatController.AddSingleTurnExternalThreatEffect(
					ThreatStatus.ReducedMovement
				);
		}
		...
		private void PerformBasicSquadLeader(Player performingPlayer)
		{
			...
			if (performingPlayer.BattleBots.IsDisabled)
				...
				SittingDuck
					.ZonesByLocation[StationLocation.ZoneLocation()]
					.RepairFirstDamage(performingPlayer);
		}

		private void PerformBasicPulseGunner(Player performingPlayer)
		{
			var pulseCannonStation = SittingDuck.StandardStationsByLocation[
				StationLocation.LowerWhite
			];
			if (
				CanFireCannon(performingPlayer)
				&& pulseCannonStation.CanFireCannon(performingPlayer)
		...
		private void PerformAdvancedSpecialization(Player performingPlayer, int currentTurn)
		{
			switch (performingPlayer.Specialization)
			{
				...
				case PlayerSpecialization.Hypernavigator:
					if (
						currentTurn == SittingDuck.Game.NumberOfTurns - 1
						|| currentTurn == SittingDuck.Game.NumberOfTurns - 2
					)
				...
				case PlayerSpecialization.Teleporter:
					...
					if (newStationLocation != null)
						performingPlayer.CurrentStation = SittingDuck.StandardStationsByLocation[
							newStationLocation.Value
						];
					break;
				default:
					throw new InvalidOperationException(
						"Missing specialization when attempting advanced specialization."
					);
			}
		}
		...
		{
			var canGoIntoSpace =
				performingPlayer.BattleBots != null
			...
			if (
				newStation != null
				&& newStation.StationLocation == StationLocation.UpperRed
				&& newStation.CanUseCharlieComponent(performingPlayer)
			)
