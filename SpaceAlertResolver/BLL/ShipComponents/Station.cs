...
namespace BLL.ShipComponents
{
	public abstract class Station
	{
		...
		protected InternalThreat GetFirstThreatOfType(
			PlayerActionType playerActionType,
			Player performingPlayer
		)
		{
			return ThreatController
				.DamageableInternalThreats.Where(threat =>
					threat.CanBeTargetedBy(StationLocation, playerActionType, performingPlayer)
				)
				.OrderBy(threat => threat.TimeAppears)
				.FirstOrDefault();
		}

		protected void DamageThreat(
			int damage,
			InternalThreat threat,
			Player performingPlayer,
			bool isHeroic
		)
		{
