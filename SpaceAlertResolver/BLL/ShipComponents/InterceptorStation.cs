...
namespace BLL.ShipComponents
{
	public class InterceptorStation : Station
	{
		...
		private void UseBattleBots(Player performingPlayer, bool isHeroic)
		{
			...
			if (firstThreat == null)
				PlayerInterceptorDamage = new PlayerInterceptorDamage(
					isHeroic,
					performingPlayer,
					StationLocation.DistanceFromShip().GetValueOrDefault()
				);
			else
		...
		public override void PerformNextPlayerAction(Player performingPlayer, int currentTurn)
		{
			...
			PerformPlayerAction(
				performingPlayer,
				GetActionPerformedInSpace(playerAction),
				currentTurn
			);
		}
		...
		{
			var actionPriority = new Queue<PlayerActionType?>(
				new List<PlayerActionType?>
				{
			...
			while (actionPriority.Any())
			{
				...
				if (
					action.FirstActionSegment.SegmentType == nextActionPriority
					|| action.SecondActionSegment.SegmentType == nextActionPriority
				)
