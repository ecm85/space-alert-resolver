...
namespace BLL.ShipComponents
{
	public abstract class Zone
	{
		...
		public IEnumerable<Player> Players =>
			UpperStation.Players.Concat(LowerStation.Players).ToList();
		...
		protected Zone(Random damageRandomizer)
		{
			...
			UnusedDamageTokenList = EnumFactory
				.All<DamageToken>()
				.Shuffle(DamageRandomizer)
				.ToList();
		}
		...
		{
			var damageDone = DebuffsBySource
				.Values.Where(debuff => debuff == ZoneDebuff.DoubleDamage)
				.Aggregate(damage, (current, doubleDamageDebuff) => current * 2);
			if (HasStructuralCarryoverDamage)
				damageDone *= 2;
			var newDamageTokens = UnusedDamageTokens
				.Take(Math.Min(damageDone, 6 - TotalDamage))
				.ToList();
			foreach (var newDamageToken in newDamageTokens)
				TakeDamage(newDamageToken);
			var extraDamageTaken =
				damageDone > newDamageTokens.Count ? damageDone - newDamageTokens.Count : 0;
		...
		{
			UpperStation.SetIneffectiveShields(
				DebuffsBySource.Values.Contains(ZoneDebuff.IneffectiveShields)
			);
			UpperStation.SetReversedShields(
				DebuffsBySource.Values.Contains(ZoneDebuff.ReversedShields)
			);
		}
		...
		public void RepairFirstDamage(Player player)
		{
			...
			var damageToRepair = damageRepairOrder.First(damage =>
				CurrentDamageTokens.Contains(damage)
			);
