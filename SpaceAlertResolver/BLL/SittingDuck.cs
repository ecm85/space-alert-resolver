...
namespace BLL
{
	public class SittingDuck : ISittingDuck
	{
		...
		)
		{
			...
			ZonesByLocation = new Zone[] { RedZone, WhiteZone, BlueZone }.ToDictionary(zone =>
				zone.ZoneLocation
			);
			InterceptorStations = new[]
			{
				interceptorStation1,
				interceptorStation2,
				interceptorStation3
		...
		{
			var interceptorComponent3 = new InterceptorsInSpaceComponent(
				this,
		...
		{
			var interceptorComponent2 = new InterceptorsInSpaceComponent(
				this,
		...
		{
			var interceptorComponent1 = new InterceptorsInSpaceComponent(
				this,
		...
		{
			var blueZone = new BlueZone(
				threatController,
				whiteZone.LowerWhiteStation.CentralReactor,
				blueDoors,
				this,
				damageRandomizer
			);
			DamageZone(initialDamage, ZoneLocation.Blue, blueZone);
			blueZone.LowerBlueStation.RocketsComponent.RocketsModified += (sender, args) =>
				RocketsModified(sender, args);
			return blueZone;
		}
		...
		)
		{
			...
			whiteZone.UpperWhiteStation.AlphaComponent.CannonFired += (sender, args) =>
				CentralLaserCannonFired(this, EventArgs.Empty);
			return whiteZone;
		}

		private static void DamageZone(
			ILookup<ZoneLocation, DamageToken> initialDamage,
		...
		public int DrainShields(IEnumerable<ZoneLocation> zoneLocations, int? amount = null)
		{
			return zoneLocations
				.Select(zoneLocation => ZonesByLocation[zoneLocation].DrainShield(amount))
				.Sum();
		}
		...
		{
			return zoneLocations
				.Select(zoneLocation => ZonesByLocation[zoneLocation].DrainReactor(amount))
				.Sum();
		}
		...
		private void TakeAttack(object sender, ThreatDamageEventArgs args)
		{
			...
			if (damage.ThreatDamageType == ThreatDamageType.ReducedByTwoAgainstInterceptors)
			{
				...
				{
					var stationsBetweenThreatAndShip = InterceptorStations.Where(station =>
						station.StationLocation.DistanceFromShip() <= damage.DistanceToSource
					);
		...
		public IEnumerable<Player> PlayersOnShip =>
			EnumFactory
				.All<StationLocation>()
				.SelectMany(stationLocation => StandardStationsByLocation[stationLocation].Players);
		...
		public void AddZoneDebuff(
			IEnumerable<ZoneLocation> zoneLocations,
			ZoneDebuff debuff,
		...
		public void RemoveZoneDebuffForSource(
			IEnumerable<ZoneLocation> zoneLocations,
			InternalThreat source
		)
		{
		...
		public void ShiftPlayersAfterPlayerActions(
			IEnumerable<ZoneLocation> zoneLocations,
			int turnToShift
		)
		{
		...
		public void ShiftPlayersAndRepeatPreviousAction(
			IEnumerable<StationLocation> stationLocations,
			int turnToShift
		)
		{
		...
		public void AddIrreparableMalfunctionToStations(
			IEnumerable<StationLocation> stationLocations,
		...
		public void TeleportPlayers(
			IEnumerable<Player> playersToTeleport,
